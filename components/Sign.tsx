...
const Sign = ({width,height,text,left,top,onPress}:SignProps) => {
    return(
        <TouchableOpacity style={{flex: 1,}} onPress={onPress}>
            <View style={styles(width,height,left,top).triangle}>
            </View>
            <View style={styles(width,height,left,top).square}>
                <Text style={styles(width,height,left,top).text}>{text}</Text>
            </View>
        </TouchableOpacity>
    );
}
...
const styles = (width:number,height:number,left:number,top:number) => StyleSheet.create({
    ...
    square:{
        ...
        height:height,
        width:width,
        borderBottomRightRadius: (height/1.4142)/8,
        borderTopRightRadius: (height/1.4142)/8,
        alignItems:'center',
